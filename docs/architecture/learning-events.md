...
| `guide_session_completed` | transición interna de `PATCH /api/guide/sessions/:id/complete` | todo: `stepsCompleted` lo cuenta el **servidor** desde el ledger de pasos aceptados (`GuideSessionStep`, avanzado solo por los comandos dedicados de paso — ADR 0019 §3-§4), no lo declara el cliente             | —                                                                                                                                                  | 1/`(guideSessionId)`                          |
...
| **4. GuideSession**                     | subdividido por ADR 0019 §10: **CC-7.4A** (ADR/diseño, docs-only) → **CC-7.4B** (catálogo + máquina de estados + ledger `GuideSessionStep`) → **CC-7.4C** (lifecycle start/step/complete + eventos) → **CC-7.4D** (OpenAPI/API client + firewall full-stack). El conteo de pasos sale EXCLUSIVAMENTE del ledger (fuente decidida en ADR 0019)                                      | cada sub-PR aditivo e independiente |
