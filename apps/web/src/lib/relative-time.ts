 ...
 *
 * "hace 3 días", "hace 1 mes", "ayer", "hoy" — designed for cards where we
 * want a quick scan, not exact timestamps.
 *
 * Uses Intl.RelativeTimeFormat when available, falls back to a simple
 * Spanish dictionary so jsdom-only test environments still get a deterministic
 * label.
 */
const MINUTE = 60;
