...
suite("C.0A · ACTIVE capability against real PostgreSQL", () => {
  ...
  it("classifies the SHIPPED schema as LINEAGE", async () => {
    ...
    //
    // C.0B2 retired the global index, so the shipped chain now classifies as
    // LINEAGE. The states BELOW still walk the whole transition — each builds
    // its own world — but this first case reports what production will
    // actually see once this train lands.
