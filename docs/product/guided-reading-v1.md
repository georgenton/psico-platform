...
```
GUIDED_READING_SPEC_VERSION=1.1
GUIDE_V1_UNIT_CLASSIFICATION=MICRO_GUIDE
GUIDED_CHAPTER_EXPERIENCE_STATUS=DESIGN_APPROVED_PENDING_IMPLEMENTATION
BOOK_EXPERIENCE_STANDARD=docs/product/book-experience-standard-v1.md
...
| 2026-07-31 | 1.1     | GR-4 made guided reading MULTI-BOOK. The reader no longer names its guide: it asks `GET /api/guide/discovery/:bookSlug/:chapterOrder` and carries the exact pin (`guideKey@guideVersion`) it gets back. The web gained per-pin registries — presentation, reader copy and the reader anchor — each looked up with `getExact`, so an unknown key or an unknown version is a `null` and never the nearest guide. Recovery and scene storage are namespaced by pin, and the panel is mounted with the pin as its React `key`: a run holds a session, a scene, a verdict and a timer that mean nothing under another guide, and an effect that cleared them would still render the stale ones for a frame. Discovery is READ-ONLY, proven by a before/after census over every table a guide can write (`DISCOVERY_DATABASE_DELTA=0`). Parejas que perduran chapter 1 — PLATFORM order 2, because the manifest gave order 1 to the preface — now offers its own guide; the OCR edition's anchor caveat is written down in the Parejas definition rather than papered over. No migration, no endpoint, no production change.                                                                                                     |
