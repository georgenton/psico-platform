...
**Estado:** Fase F (UI V2 + L2/L3) · **Fecha:** 2026-07-11
...
**G:** retiro del layout legacy (borra las ramas legacy de MapStage/MapDims/mapa mobile → el snapshot del copy-ratchet baja a cero) + tratamiento V2 de la serie «Comprensión emocional» de Evolución (hoy `pct` sigue alimentando ese chart).
...
| L3  | Provider LLM → Narrator (solo copy) vs eliminar                   | Narrator opcional apagable                                  | ✅ aprobada e implementada (Fase F): score jamás bajo V2 · NAR-L1 con `EMOTIONAL_MAP_NARRATOR` default off                                                            |
