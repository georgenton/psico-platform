...
/**
 * The reader, addressed by a chapter's STABLE identity.
 *
 * Lifted out of `app/.../lector/[chapterOrder].tsx` so the canonical route and
 * the positional compatibility route render the SAME screen. A second copy
 * would be a second place for source-selection, entitlement and mark-writing
 * rules to drift apart — and the two would look identical until they did not.
 *
 * `chapter.order` from the envelope is still used freely below for labels and
 * for the position-based media/guide endpoints. What changed is that no
 * NAVIGATION is generated from it.
 */
export function LectorScreen({
  slug,
  readerRef,
}: {
  slug: string;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        data = await lectorApi.getChapterByRef(slug ?? "", readerRef);
      } catch (e: unknown) {
        if (cancelled) return;
        const status =
  ...
  async function handleComplete() {
    ...
    try {
      ...
      if (res.nextReaderRef) {
        router.replace(
          readerRoutePath(chapter.book.slug, res.nextReaderRef) as never,
        );
      } else {
        router.back();
      }
