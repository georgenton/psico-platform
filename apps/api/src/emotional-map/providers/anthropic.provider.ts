...
export class AnthropicEmotionalMapProvider implements IEmotionalMapProvider {
  ...
  async score(
    ...
    const userPrompt = [
      ...
      ``,
      `DATOS (agregados, sin texto del diario):`,
      `- Entradas últimos 30 días: ${payload.stats.entryCount}`,
      `- Días activos en la ventana: ${payload.stats.activeDays}`,
      ...
      ``,
      `EJES:`,
      `- calma: estabilidad emocional. Más alta cuando hay variedad de mood sin dominancia de ánsiedad/hard.`,
      `- claridad: capacidad de nombrar lo que se siente. Más alta con tags categoriales (trabajo, familia, sueño) y consistencia.`,
      `- compasion: tono validante hacia sí mismo. Más alta cuando hay entries en moods difíciles SIN abandono (sigue escribiendo).`,
      `- consciencia: regularidad de la observación de sí mismo. Más alta con activeDays altos y distribución pareja.`,
      ``,
