...
export class CirclesService {
  ...
  async inspect(
    presented: string,
    now: Date = new Date(),
  ): Promise<CircleInvitationPreview> {
   ...
   *
   * Any failure here is the same refusal as a bad secret. An invitation we
   * cannot describe — a pin this build does not carry, an inviter row that
   * vanished — is one we should not be inviting anybody to accept, and saying
   * so specifically would confirm the invitation is real.
   */
  private async previewOf(invitation: {
    ...
    let member;
    try {
      [activity, member] = await Promise.all([
    ...
    if (!activity || !member) throw unusable;

    let definition;
    try {
      definition = productionCircleTemplateRegistry.getExact(
