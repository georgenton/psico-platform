 ...
 * recovery / stability in [0,1]) into warm, human Spanish. This is the
 * "hybrid" layer: the radar + numbers stay for identity, but the FIRST thing
 * the user reads is a sentence about themselves, not a parameter.
 *
 * Buckets are presentation thresholds (validated against the persona
 * benchmark: demo-estable → high/fast/variable, demo-volatil → volatile,
 * casi-plano → steady). Non-diagnostic framing always — no pathologizing
 * language, low-mood copy stays kind.
 */
 ...
 * Recovery may still be gated (null) — its row degrades to a gathering note.
 */
